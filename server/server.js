...
app.get("/inventory/favGameAndGenre/:ID", (req, res) => {
    const ID = req.params.ID;
    const sqlPut = "SELECT igre.naziv As FavGame, kategorija.naziv AS FavKat FROM korisnik INNER JOIN igre ON korisnik.favGame = igre.ID INNER JOIN kategorija ON korisnik.favGenre = kategorija.ID WHERE korisnik.id = ?"
